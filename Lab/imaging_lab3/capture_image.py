...
def serial_ports():
  ...
  if sys.platform.startswith('win'):
      ports = ['COM' + str(i + 1) for i in range(512)]
...
class Mask(QtGui.QWidget):
  ...
  def initUI(self):
     ...
     EE16A_SCAN_DELAY_USER = int(input("How much delay would you like ser scan? (ms): "))
  ...
  def updateData(self):
    ...
    if self.count < self.N*self.M:
      if self.fp_flag < math.floor(EE16A_SCAN_DELAY/(100.0 + EE16A_SCAN_DELAY_USER)):
        self.fp_flag += 1
        self.count = 0

      elif self.fp_flag == math.floor(EE16A_SCAN_DELAY/(100.0 + EE16A_SCAN_DELAY_USER)):
      ...
      np.save("%s%d.npy"%(self.out_file, self.runs), np.concatenate((np.array(self.sensor_readings_final),np.array(extra_data))))
      self.runs += 1
  ...
  # Load the imaging mask from "imaging_mask.npy"
        ...
        print("Starting scan...")
        self.started=True
        self.timer.start(100.0 + EE16A_SCAN_DELAY_USER)
