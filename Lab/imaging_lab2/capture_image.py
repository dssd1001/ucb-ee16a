...
class Mask(QtGui.QWidget):
  ...
  def initUI(self):
     ...
     EE16A_SCAN_DELAY_USER = int(input("How much delay would you like per scan? (ms): "))
  ...
  def updateData(self):
    ...
    if self.count < self.N*self.M:
      if self.fp_flag <EE16A_SCAN_DELAY/(100.0 + EE16A_SCAN_DELAY_USER):
        self.fp_flag += 1
        self.count = 0

      elif self.fp_flag == EE16A_SCAN_DELAY/(100.0 + EE16A_SCAN_DELAY_USER):
